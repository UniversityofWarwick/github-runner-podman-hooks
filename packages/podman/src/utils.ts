...
const PODMAN_LOGLEVEL = env['PODMAN_LOGLEVEL']
...
export async function runDockerCommand(
  ...
  args = fixArgs(args)
  if (PODMAN_LOGLEVEL) {
    args.unshift(`--log-level=${PODMAN_LOGLEVEL}`)
  }
  core.debug(`Running podman (HOME=${env.HOME}, PATH=${env.PATH})}`)
  const pipes = await exec.getExecOutput('podman', args, options)
  if (pipes.exitCode !== 0) {
