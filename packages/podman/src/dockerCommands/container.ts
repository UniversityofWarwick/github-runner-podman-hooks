...
function setMountVolumeOptions(dockerArgs: string[], args: ContainerInfo): void {
  ...
  for (const mountVolume of mountVolumes) {
    ...
    if (!fs.existsSync(mountVolume.sourceVolumePath)) {
      core.info(`Creating source volume path ${mountVolume.sourceVolumePath}`)
      fs.mkdirSync(mountVolume.sourceVolumePath, { recursive: true })
    }
  }
}
